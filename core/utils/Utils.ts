...
import { ARRAY_OPERATORS, JSON_KEY_OPERATORS, GroupOperator, PlainObject, QueryOperator, ReferenceKind } from '../enums.ts';
...
export class Utils {
   ...
   */
  static absolutePath(path: string, baseDir = Deno.cwd()): string {
    if (!path) {
   ...
   */
  static requireFrom<T extends Dictionary>(id: string, from = Deno.cwd()): T {
    if (!extname(from)) {
