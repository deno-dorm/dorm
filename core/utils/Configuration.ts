...
import type { NamingStrategy } from '../naming-strategy/index.ts';
import type { CacheAdapter } from '../cache/index.ts';
import type { EntityRepository } from '../entity/EntityRepository.ts';
import type {
  AnyEntity,
  Constructor,
  Dictionary,
...
import { ObjectHydrator } from '../hydration/index.ts';
import { NullHighlighter } from '../utils/NullHighlighter.ts';
import { DefaultLogger, colors, type Logger, type LoggerNamespace, type LoggerOptions } from '../logging/index.ts';
import { Utils } from '../utils/Utils.ts';
import type { EntityManager } from '../EntityManager.ts';
import type { Platform } from '../platforms/mod.ts';
import type { EntitySchema } from '../metadata/EntitySchema.ts';
import type { MetadataProvider } from '../metadata/MetadataProvider.ts';
import type { MetadataStorage } from '../metadata/MetadataStorage.ts';
import type { EmbeddedPrefixMode } from '../decorators/Embedded.ts';
import type { EventSubscriber } from '../events/index.ts';
import type { AssignOptions } from '../entity/EntityAssigner.ts';
import type { EntityManagerType, IDatabaseDriver } from '../drivers/IDatabaseDriver.ts';
import { NotFoundError } from '../errors.ts';
import { RequestContext } from './RequestContext.ts';
import { DataloaderType, FlushMode, LoadStrategy, PopulateHint } from '../enums.ts';
import { MemoryCacheAdapter } from '../cache/MemoryCacheAdapter.ts';
...
export class Configuration<D extends IDatabaseDriver = IDatabaseDriver, EM extends EntityManager = D[typeof EntityManagerType] & EntityManager> {
  ...
  getMetadataProvider(): MetadataProvider {
    return this.getCachedService(TsMorphMetadataProvider, this);
  }
  ...
  private detectSourceFolder(options: Options): void {
    if (!false) { //TODO: Remove this detection for TS
      return;
    }

    const migrationsPathExists = false;
    const seedersPathExists = false;
    const distDir = false;
    const buildDir = false;
