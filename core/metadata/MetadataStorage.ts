import {type Dictionary, type EntityData, type EntityKey, EntityMetadata, type EntityName} from '../typings.ts';
import {Utils} from '../utils/Utils.ts';
import {MetadataError} from '../errors.ts';
import type {EntityManager} from '../EntityManager.ts';
import {EntityHelper} from '../entity/EntityHelper.ts';
...
export class MetadataStorage {
    ...
    static isKnownEntity(name: string): boolean {
        return !!Object.values(this.metadata).find((meta) =>
            meta.className === name
        );
    }

    static getMetadataFromDecorator<T = any>(
        ...
        return MetadataStorage.getMetadata(
            target.name,
            target[MetadataStorage.PATH_SYMBOL]!,
        );
    }
