...
import type { AnyEntity, Constructor, Dictionary, EntityMetadata, EntityProperty, IPrimaryKey } from './typings.ts';
...
export class MetadataError<T extends AnyEntity = AnyEntity> extends ValidationError<T> {
  ...
  static onlyAbstractEntitiesDiscovered(): MetadataError {
    return new MetadataError('Only abstract entities were discovered, maybe you forgot to use @Entity() decorator? This can also happen when you have multiple `@dorm/core` packages installed side by side.');
  }
